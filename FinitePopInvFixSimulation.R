##### parameters  #####
source('finite.pop.functions.R')
source('finite.pop.generation.R')
...
# selection on sal
s <- .25 #sel
t <- .25 # reduction due to sex geno mismatch
gs <- .25  # selection on gfl
...
for(n in 2:length(N.vals)){
      ...
      for(k in 1:1000){
        ...
        while(done == F){
          ...
          for(m in 1:length(pop)){
            ...
            if(dubinv == T){
              geno.sex <- paste(substr(pop[m], 3, 3), substr(pop[m], 9, 9))
            }else{
              geno.sex <- paste(substr(pop[m], 3, 3), substr(pop[m], 8, 8))
            }
            
            if(geno.sex == 'Y Y'){ y <- y + 2}
            if(geno.sex == 'Y X' | geno.sex == 'X Y'){y <- y + 1}
  ...
  for(i in 1:length(rates)){
    ...
    for(j in 1:length(cd.vals)){
      cd <- cd.vals[j]
      # invfix is as before the number of times the inv fixes in a pop
      invfix <- 0
      cat('\014')
