...
mutate <- function(pop, rate, rectable){
  ...
  for(i in 1: length(pop)){
    ...
    if(runif(1) < 2 *rate){
      sex <- substr(pop[i],1,1)
      inv <- nchar(pop[i]) == 12
      dubinv <- nchar(pop[i]) == 13
      # because characters are shifted around with inversions
      if(dubinv == T){
        hap1 <- substr(pop[i], 3, 6)
        hap2 <- substr(pop[i], 9, 12)
      }else if(inv == T){
        hap1 <- substr(pop[i], 3, 6)
        hap2 <- substr(pop[i], 8, 11)
      }else{
        hap1 <- substr(pop[i], 3, 6)
        hap2 <- substr(pop[i], 8, 11)
      }
...
fitnessSelection <- function(pop,fits){
  ...
  new.males <- sample(males, length(males), replace = T, prob = male.fits)
  new.females <- sample(females, length(females), replace = T, prob = female.fits)
  # combining to return
  new.pop <- list(new.males, new.females)
  names(new.pop) <- c('males', 'females')
  return(new.pop)
}
